...
| z.ai | User-created API token | Enabled for the fixed Global personal-quota endpoint; BigModel China/team mode is deferred | Token is represented only by a provider-scoped fingerprint outside encrypted storage | Token is value-encrypted, never logged, validated before save, and never sent across redirects | Reads Tokens, MCP/time, and rolling token limits from `api.z.ai/api/monitor/usage/quota/limit`; no endpoint override is accepted |
...
| Cline | User-created API key | Enabled for the fixed Cline plan-usage endpoint | The key is represented only by a provider-scoped fingerprint outside encrypted storage | Key is value-encrypted, never logged, validated before save, and never sent across redirects | Reads provider-reported 5-hour, weekly, and monthly windows from `api.cline.bot/api/v1/users/me/plan/usage-limits` |
| IBM Bob | User-created API key or JWT | Enabled for profile-visible teams | The credential is represented only by a provider-scoped fingerprint outside encrypted storage | Credential is value-encrypted; valid JWT shape selects Bearer while other keys use Apikey; neither value is logged | Reads profile and team budgets only from HTTPS hosts strictly under `bob.ibm.com`; dynamic regional hosts are validated against URL-component and suffix-bypass attacks |
